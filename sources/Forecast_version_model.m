...
%% creation de la fonction de calcul de risque sur mesure pour la station considérée 
resultDir = fullfile(fileparts(mfilename('fullpath')),'..','results');
if ~isfolder(resultDir), mkdir(resultDir); end
outPath = fullfile(resultDir,'calcrisk_c.c');

% Vérifications minimales
if ~exist('b_final','var') || ~exist('mu','var') || ~exist('sigma','var')
    error('Variables b_final, mu et sigma doivent exister dans le workspace.');
end
b = b_final(:)';           % ligne
mu = mu(:)'; sigma = sigma(:)';

nX = numel(mu);
% b doit avoir nX+1 si on a un terme constant b0
if ~(numel(b) == nX+1)
    error('Taille de b_final incompatible : attendu %d (= nvars+1), trouvé %d', nX+1, numel(b));
end

fid = fopen(outPath,'w','n','UTF-8');
if fid == -1, error('Impossible d''ouvrir %s en écriture.', outPath); end

fprintf(fid, "/* calcrisk_c.c\n");
fprintf(fid, "   Generated by write_calcrisk_c.m\n");
fprintf(fid, "   Signature:\n");
fprintf(fid, "     double calcrisk_c(const double *x, int n);\n");
fprintf(fid, "   - x must contain %d features in the exact order used to build X in MATLAB\n", nX);
fprintf(fid, "   - returns the risk index (double)\n");
fprintf(fid, "*/\n\n");
...
% write mu and sigma arrays
fprintf(fid, "static const double MU[%d] = {", nX);
fprintf(fid, "%g", mu(1));
for k=2:nX, fprintf(fid, ", %g", mu(k)); end
fprintf(fid, "};\n");

fprintf(fid, "static const double SIGMA[%d] = {", nX);
fprintf(fid, "%g", sigma(1));
for k=2:nX, fprintf(fid, ", %g", sigma(k)); end
fprintf(fid, "};\n\n");

% write coefficients (assuming b(1) is intercept)
fprintf(fid, "static const double B[%d] = {", nX+1);
fprintf(fid, "%g", b(1));
for k=2:(nX+1), fprintf(fid, ", %g", b(k)); end
fprintf(fid, "};\n\n");

% function
fprintf(fid, "double calcrisk_c(const double *x, int n)\n{\n");
fprintf(fid, "    if (x == NULL) return 0.0;\n");
fprintf(fid, "    if (n != N_FEATURES) return 0.0; /* caller must pass correct length */\n\n");
fprintf(fid, "    /* compute normalized features and linear model with intercept */\n");
fprintf(fid, "    double sum = B[0]; /* intercept */\n");
...
fclose(fid);
fprintf('Fichier C généré : %s\n', outPath);

%% test de la fonction calcrisk
